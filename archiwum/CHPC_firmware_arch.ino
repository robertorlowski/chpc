...
#define MAGIC     		    0x48   		//change if u want to reinit T sensors
...
unsigned long millis_displ_update_interval 	= 3000;//10000;
...
void Print_D2 (String outString, int line =1) {
	#ifdef   DISPLAY_1602   
...
void SaveSetpointEE(int pforce = 0) {
	if(  	(T_setpoint_lastsaved != T_setpoint) && 
		( pforce == 1 || ((unsigned long)(millis_now - millis_lasteesave) > 15*60*1000 )  ||  (millis_lasteesave == 0) )  ) {
			eeprom_addr = 1;
...
void loop(void) {  
    ...
    if ( (z == 1) && ( i == 1) ) {
      //
        ...
        if (x == 1) {
          PrintS_and_D("Ustaw T(z): " + String(T_setpoint));
          delay(300);
          SaveSetpointEE(1);
        }
        ...
        if (d == 1) {
          Inc_Tdelta();
          PrintS_and_D("Ustaw dT: " + String(T_delta));
          delay(300);
      ...
      //process_hot_side_pump:
			...
			if (  heatpump_state == 1   &&  
				(	errorcode != 0                             		||
					...
					(Tco.e 	== 1 	&& 	Tco.T 	< cT_cold_min) )     ) {
						#ifdef RS485_HUMAN 
							PrintS(String("Protective STOP") + " Err.:" + String(errorcode, HEX));
						#endif
						millis_last_heatpump_off = millis_now;
						heatpump_state = 0;
						//digitalWrite(RELAY_HEATPUMP, heatpump_state);  // old, now halifised
			...
			if (  heatpump_state == 1   &&  ((unsigned long)(millis_now - millis_last_heatpump_on) > MINCYKLE_CHECK )  ) {
				...
				if ( ( errorcode == ERR_OK )     &&   (  Tsump.e == 1 && Tsump.T < cT_workingOK_sump_min )  ) {
					...
					heatpump_state = 0;
          #ifdef RS485_HUMAN 
					  PrintS("Error HEATPUMP less than MIN: " + String(Tsump.T));
				  #endif
				}
				if ( ( errorcode == ERR_OK )     &&   ( async_wattage < c_workingOK_wattage_min )  ) {
					...
					heatpump_state = 0;
          #ifdef RS485_HUMAN 
					  PrintS("Error WATTAGE less than MIN: " + String(async_wattage));
				  #endif
				}
				//digitalWrite(RELAY_HEATPUMP, heatpump_state);	// old, now halifised
			}
