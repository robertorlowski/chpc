...
| [`test_chpc_debuglog`](test_chpc_debuglog/test_main.cpp) | 5 | tylko `pio test -e native_debug` (build z `DEBUG_LOG`): każda linia logu jest obiektem JSON; zdarzenia `boot`, `lcd`, `eev`, `st`, `btn` (z odczytem ADC wejścia A1), `rel`, `err`; odpowiedź na `0x01` nadal działa |
